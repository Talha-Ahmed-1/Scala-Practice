...
object second {
  ...
  def main(args: Array[String]) {
    ...
    import FrontEnd._
//    var html1 = UsersDAO.find(1) //use map & fold
//    var html2 = UsersDAO.find(2) //use map & fold

    val html1 = UsersDAO.find(1).map(userToHTML).fold(displayError("There is no user with this id"))(displayPage)
    val html2 = UsersDAO.find(2).map(userToHTML).fold(displayError("There is no user with this id"))(displayPage)

//    val res = html1.map(userToHTML)
//    html1 = res.fold(displayError("There is no user with this id"))(displayPage)
////    println(res.fold(displayError("There is no user with this id"))(displayPage))
//    val res1 = html2.map(userToHTML)
//    html2 = res1.fold(displayError("There is no user with this id"))(displayPage)
//    println(res1.fold(displayError("There is no user with this id"))(displayPage))
//    val html1 = UsersDao.map(1)
//    val id : Option[Int] = Some(1)
//    val res = id.map(UsersDAO.find)
//
//    val extra: Any = UsersDAO.find(1)
//    val extra1: Int = 1
//    res.get.fold(extra1)(extra)

    println("[ID=2] - below one line with domain transformation")
//    UsersDAO.find(2) //use map & orElse & foreach to display result
//    val html3 = UsersDAO.find(2).map(userToHTML).getOrElse(displayError("There is no user with this id"))//use map & getOrElse

    UsersDAO.find(2).map(userToHTML).map(displayPage).orElse(Some(displayError("There is no user with this id"))).foreach(println)
    val html3 = UsersDAO.find(1).map(userToHTML).map(displayPage).getOrElse(displayError("There is no user with this id"))
