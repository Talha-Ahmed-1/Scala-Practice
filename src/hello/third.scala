...
import hello.third.Exercise.UsersDAO
...
object third {
  ...
  object Exercise {
    ...
    object Conversions {
      ...
      //EXERCISE
      def meetupHistoryToHTML(mh: MeetupHistory) = s"<h1>${mh.title}</h1><img>${mh.picture}</img>"
    }
    ...
    object Controller {
      ...
      //EXERCISE
//      def extra(a: MeetupHistory):

      def userLastMeetup(id: Int): HTML = UsersDAO.find(id).fold(displayError("There is no id."))(handling)
      def handling(user: User): HTML = UsersDAO.find(user.id).flatMap(_.history.headOption).map(meetupHistoryToHTML).fold(displayError("NEW USER : 0 MEETUPS"))(displayPage)
